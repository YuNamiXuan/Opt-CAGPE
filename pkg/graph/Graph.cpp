...
void Graph::init_serializer(std::string path, MODE mode)
{
    ...
    {
        cache_mode = CACHE_MODE::NO_CACHE;
    }
...
void Graph::reset_cache()
{
    ...
    if (cache_mode == CACHE_MODE::SIMPLE_CACHE)
        simple_cache = new SimpleCache<EdgeBlock>(&gs, num_cache_blocks);
    else if (cache_mode == CACHE_MODE::NORMAL_CACHE)
...
void Graph::disable_cache() { cache_mode = CACHE_MODE::NO_CACHE; }

void Graph::clear_cache()
{
    if (cache_mode == CACHE_MODE::NO_CACHE)
        return;
    else if (cache_mode == CACHE_MODE::SIMPLE_CACHE)
        simple_cache->clear();
    else
        edge_cache->clear();
}
...
uint32_t Graph::get_cache_block_idx(uint32_t eb_id)
{
    ...
    int cb_idx = simple_cache->request_block(block_id, active_vid_in_eb[eb_id].size());
...
{
    this->set_active_vertices(frontier);
    pool.push_loop(active_edge_blocks.size(), [this, &next, &update](const int a, const int b)
                   {
    for (int i = a; i < b; i++) {
      auto eb_id = active_edge_blocks[i];
      auto cb_idx = this->get_cache_block_idx(eb_id);
